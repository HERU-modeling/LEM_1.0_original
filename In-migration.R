...
mat   = vparameters$mat*12 # read yearly maturation rate data
...
mig.r.out = numeric(18)
mig.out.seq = c(2, 6, 10, 3, 7, 11, 1, 5, 9, 13, 15, 17, 4, 8, 12, 14, 16, 18)
mig.r.out[mig.out.seq] = mig.r[1:18]

# copy the result to excel
write.excel <- function(tab, ...) write.table( tab, "clipboard", sep="\t", row.names=F)
write.excel(mig.r.out)
