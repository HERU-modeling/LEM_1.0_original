...
#vparameters$calipsi = T 
nm.psi <- nmkb(par = c(psi18, vparameters$v2, vparameters$v3, vparameters$high.psi), fn =psiObj, 
               lower =c(rep(0, 18),0.003, 0.008, 1), upper =c(rep(0.15, 18), 0.01, 0.03, 2),
               fixed = vparameters, obs = ndiag18.obs[2, ])

#par = c(psi18, vparameters$v2, vparameters$v3, vparameters$high.psi); fixed = vparameters; obs = ndiag18.obs[2, ]
