...
## Function set-up ##
rm(list=ls())
setwd("C:/Users/Xiao/Dropbox/R code/Casacades model R code/Cascades model-HERU_2/Morris method")
#setwd("C:/Users/xza162/Dropbox/R code/Casacades model R code/Cascades model-HERU_2/Morris method")
...
set.seed(54)            #May the force be with you
for (i in 1:r){
...
for (i in 1:r){
  MMresult <- matrix(0, nrow =k+1, ncol =nt)
  pj = 0
  ...
  ndiagy       = c(ndiag.all[4], ndiag.b[4], ndiag.m[4])
  ...
  deathy       = c(death.all[4], death.b[4], death.m[4])
  
  #### Combining all results
  MMresult[1, ] <- c(diagc, ndiagy, deathy)
  ...
  for (pj in 1:k){
    ...
    MMresult[pj+1, ] <- c(diagc, ndiagy, deathy)
    #MMresult[pj+1,]<-result[pj+1,,r]
    
    if (deltaarray[pj+1, 2, i] >0){
      EEarrary[deltaarray[pj+1, 1, i], ,i] <- ((MMresult[pj+1, ] - MMresult[pj, ])   /(deltaarray[pj+1, 3, i]))
    }
    else {
      EEarrary[deltaarray[pj+1, 1, i], ,i] <- ((MMresult[pj, ]   - MMresult[pj+1, ]) /(deltaarray[pj+1, 3, i]))
    }
  }
}
...
tar2015 <- c(diag11.obs[4, ], ndiag.all.obs[4], ndiag.b.obs[4], ndiag.m.obs[4], death.all.obs[4], death.b.obs[4], death.m.obs[4])
tar2015 <- rep(tar2015, each=npar)

results <- cbind(results, tar2015)
#Choose the value for mean and stddev larger to which parameter will be labelled
#ggplot(data =results, aes(x =mean, y =stddev)) + geom_point() + labs(x="Mean of elementary effect", y="Standard deviation of elementary effect") + facet_wrap(~target, ncol=3, scales="free") + geom_text(aes(label=ifelse(mean>0.15*tar2015 | stddev>0.15*tar2015, as.character(parameter), '')), hjust=1, vjust=0.5, size=2.5)

#results[results$target=="New diag, M black HET" & results$mean>5,]

influential0 = subset(results, mean>0.05*tar2015 | stddev>0.05*tar2015)
length(unique(influential0$parameter))

influential1 = subset(results, mean>0.1*tar2015 | stddev>0.1*tar2015)
length(unique(influential1$parameter))

influential2 = subset(results, mean>0.15*tar2015 | stddev>0.15*tar2015)
length(unique(influential2$parameter))

influential3 = subset(results, mean>0.20*tar2015 | stddev>0.20*tar2015)
length(unique(influential3$parameter))

influential4 = subset(results, mean>0.25*tar2015 | stddev>0.25*tar2015)
length(unique(influential4$parameter))

influential5 = subset(results, mean>0.30*tar2015 | stddev>0.30*tar2015)
length(unique(influential5$parameter))

influential6 = subset(results, mean>0.50*tar2015 | stddev>0.50*tar2015)
length(unique(influential6$parameter))

write.excel <- function(x, row.names=FALSE, col.names=TRUE,...) {
  write.table(x,"clipboard",sep="\t", row.names=row.names, col.names=col.names, ...)
}

write.excel(influential6)
